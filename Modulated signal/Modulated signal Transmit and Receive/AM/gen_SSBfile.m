function  gen_SSBfile(signal,rxSignal,dataform,datatype)
%% gen filename 
if signal.type=="LSB"||signal.type=="USB"
    filename=[str2mat(signal.type),'_',num2str(signal.gen_method),'_fs',num2str(signal.am.fs),'_IFfs',num2str(signal.am.IFfs),'_fc',num2str(signal.am.fc), ...
        '_Foffset',num2str(signal.am.f_offset),'_SNR',num2str(signal.noise),dataform];
    filename2=[str2mat(signal.type),'_',num2str(signal.gen_method),'_fs',num2str(signal.am.fs),'_IFfs',num2str(signal.am.IFfs),'_fc',num2str(signal.am.fc), ...
...
signal_para=[tmp0,tmp,signal.am.fs,signal.am.IFfs,signal.am.fc,signal.am.fc_amp,signal.am.f_offset,...
