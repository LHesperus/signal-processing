...
II_m=real(IQ_in)/mean(abs(IQ_in));
QQ_m=imag(IQ_in)/(mean(abs(IQ_in)));
% 定时
...
for m=1:(length(II_m)-5)
    ...
    if q_temp(m+1)<0
        ...
        if strobe==1
            if k>2
               time_error(s)=yi_I(k-1)*(sign(yi_I(k))-sign(yi_I(k-2)))+yi_Q(k-1)*(sign(yi_Q(k))-sign(yi_Q(k-2))); 
...
ni=max(yi_I);
nq=max(yi_Q);
yi_II=yi_I(1:2:end)/ni;
yi_QQ=yi_Q(1:2:end)/nq;
